...
final class CommunicationSeederDefaults implements ServiceContract
{
  ...
  public function allTemplates(): array
  {
    ...
    return array_merge([
      ...
      'form.membership.submitted' => $this->tpl(
        ...
        '<p>Hello {{applicant_name}},</p><p>Thank you for applying to join Marketplace Ministers. We received your application <strong>{{application_number}}</strong>. Our team will review it and contact you about the next step.</p><p>You can track your application status here: <a href="{{status_url}}">{{status_url}}</a></p>',
        $vars(['applicant_name', 'email', 'application_number', 'status_url']),
        ['applicant_name' => 'Applicant Name', 'application_number' => 'APP-1001', 'status_url' => 'https://example.com/membership/status'],
      ),
  ...
  private function extendedTemplates(): array
  {
    ...
    return [
      ...
      'counseling.payment.required' => $this->tpl('counseling-payment-required', 'Counselling payment required', 'counseling', 'counseling.payment.required', 'Payment required for counselling — {{case_number}}', '<p>Hello {{applicant_name}},</p><p>Payment of {{amount}} {{currency}} is required to continue case {{case_number}}.</p>', $v(['applicant_name', 'case_number', 'amount', 'currency', 'payment_reference']), ['amount' => '50.00', 'currency' => 'USD']),
      'counseling.payment.received' => $this->tpl('counseling-payment-received', 'Counselling payment received', 'counseling', 'counseling.payment.received', 'Payment received — {{case_number}}', '<p>Hello {{applicant_name}},</p><p>We received your payment of {{amount}} {{currency}} for case {{case_number}}.</p>', $v(['applicant_name', 'case_number', 'amount', 'currency', 'payment_reference']), ['amount' => '50.00', 'currency' => 'USD']),
      'lms.assignment.submitted' => $this->tpl('lms-assignment-submitted', 'Assignment submitted', 'learning', 'lms.assignment.submitted', 'Assignment submitted — {{assignment_title}}', '<p>Hello {{member_name}},</p><p>Your assignment <strong>{{assignment_title}}</strong> for {{course_name}} has been received.</p>', $v(['member_name', 'assignment_title', 'course_name']), ['assignment_title' => 'Reflection Paper']),
      'membership.application.approved' => $this->tpl('membership-application-approved', 'Membership application approved', 'membership', 'membership.application.approved', 'Your membership application is approved', '<p>Hello {{applicant_name}},</p><p>Congratulations — your membership application <strong>{{application_number}}</strong> has been approved. Watch for login credentials and onboarding next steps.</p>', $v(['applicant_name', 'email', 'application_number', 'reason']), ['applicant_name' => 'Member']),
      'membership.request.more_information' => $this->tpl('membership-request-more-information', 'More information requested', 'membership', 'membership.request.more_information', 'More information needed for your membership application', '<p>Hello {{applicant_name}},</p><p>The membership team needs more information about your application.</p><p>{{message}}</p>', $v(['applicant_name', 'email', 'application_number', 'message']), []),
      'membership.interview.invitation' => $this->tpl('membership-interview-invitation', 'Membership interview invitation', 'membership', 'membership.interview.invitation', 'Your membership interview invitation', '<p>Hello {{applicant_name}},</p><p>Your membership interview has been scheduled.</p><p>Date: {{event_date}} {{event_time}}</p><p><a href="{{confirmation_url}}">Confirm attendance</a></p>', $v(['applicant_name', 'event_date', 'event_time', 'confirmation_url', 'meeting_url', 'physical_location']), []),
      'membership.interview.rescheduled' => $this->tpl('membership-interview-rescheduled', 'Membership interview rescheduled', 'membership', 'membership.interview.rescheduled', 'Your membership interview was rescheduled', '<p>Hello {{applicant_name}},</p><p>Your membership interview has been rescheduled.</p>', $v(['applicant_name', 'event_date', 'event_time', 'confirmation_url']), []),
      'membership.interview.confirmed' => $this->tpl('membership-interview-confirmed', 'Membership interview confirmed', 'membership', 'membership.interview.confirmed', 'Interview attendance confirmed', '<p>Hello {{applicant_name}},</p><p>Thank you — your interview attendance is confirmed.</p>', $v(['applicant_name', 'event_date', 'event_time']), []),
      'membership.interview.reminder' => $this->tpl('membership-interview-reminder', 'Membership interview reminder', 'membership', 'membership.interview.reminder', 'Reminder: upcoming membership interview', '<p>Hello {{applicant_name}},</p><p>This is a reminder of your upcoming membership interview.</p>', $v(['applicant_name', 'event_date', 'event_time']), []),
      'membership.interview.passed' => $this->tpl('membership-interview-passed', 'Membership interview passed', 'membership', 'membership.interview.passed', 'You passed your membership interview', '<p>Hello {{applicant_name}},</p><p>Great news — you passed your membership interview. Onboarding will continue shortly.</p>', $v(['applicant_name', 'application_number']), []),
      'membership.interview.failed' => $this->tpl('membership-interview-failed', 'Membership interview outcome', 'membership', 'membership.interview.failed', 'Membership interview outcome', '<p>Hello {{applicant_name}},</p><p>Thank you for attending your membership interview. The membership team will follow up regarding next steps.</p>', $v(['applicant_name', 'application_number']), []),
      'membership.interview.awaiting_review' => $this->tpl('membership-interview-awaiting-review', 'Interview awaiting review', 'membership', 'membership.interview.awaiting_review', 'Interview awaiting review — {{application_number}}', '<p>This interview is awaiting review. Please record the outcome in the admin workspace.</p><p>Applicant: {{applicant_name}} ({{email}})</p>', $v(['applicant_name', 'email', 'application_number']), []),
      'membership.interview.cancelled' => $this->tpl('membership-interview-cancelled', 'Membership interview cancelled', 'membership', 'membership.interview.cancelled', 'Membership interview cancelled', '<p>Hello {{applicant_name}},</p><p>Your membership interview has been cancelled.</p>', $v(['applicant_name']), []),
      'membership.account.created' => $this->tpl('membership-account-created', 'Member account credentials', 'membership', 'membership.account.created', 'Your Marketplace Ministers login credentials', '<p>Hello {{applicant_name}},</p><p>Your Member Workspace account has been created.</p><p>Username: {{username}}</p><p><a href="{{activation_url}}">Activate your account</a></p><p>Login: <a href="{{login_url}}">{{login_url}}</a></p>', $v(['applicant_name', 'email', 'username', 'activation_url', 'login_url', 'temporary_password']), []),
      'membership.account.upgraded' => $this->tpl('membership-account-upgraded', 'Account upgraded to member', 'membership', 'membership.account.upgraded', 'Your account is now a Member Workspace login', '<p>Hello {{applicant_name}},</p><p>Your existing account now has Member Workspace access.</p><p><a href="{{login_url}}">Sign in</a></p>', $v(['applicant_name', 'email', 'username', 'login_url']), []),
      'membership.welcome' => $this->tpl('membership-welcome', 'Member welcome', 'membership', 'membership.welcome', 'Welcome to Marketplace Ministers', '<p>Hello {{applicant_name}},</p><p>Welcome to the Marketplace Ministers tribe. Your Member Portal and Learning Portal are ready.</p>', $v(['applicant_name', 'email', 'login_url']), []),
      'membership.ministry.onboarding' => $this->tpl('membership-ministry-onboarding', 'Ministry and country onboarding', 'membership', 'membership.ministry.onboarding', 'Your ministry and country onboarding links', '<p>Hello {{applicant_name}},</p><p>Welcome to {{ministry_name}}{{country_name}}.</p><p>Use the community links shared by your ministry team to complete onboarding.</p>', $v(['applicant_name', 'ministry_name', 'country_name', 'whatsapp_link', 'telegram_link']), []),
      ...
      'event.announcement' => $this->tpl('event-announcement', 'Event announcement', 'events', 'event.announcement', '{{announcement_subject}}', '<p>Hello {{applicant_name}},</p><p>{{announcement_body}}</p>', $v(['applicant_name', 'event_name', 'announcement_subject', 'announcement_body']), []),
      'event.certificate.issued' => $this->tpl('event-certificate-issued', 'Event certificate issued', 'events', 'event.certificate.issued', 'Your event certificate is ready', '<p>Hello {{applicant_name}},</p><p>Your certificate for <strong>{{event_name}}</strong> is ready. Verification code: {{verification_code}}</p>', $v(['applicant_name', 'event_name', 'certificate_number', 'verification_code']), []),
      'event.check_in.reminder' => $this->tpl('event-check-in-reminder', 'Event check-in reminder', 'events', 'event.check_in.reminder', 'Check in for {{event_name}}', '<p>Hello {{applicant_name}},</p><p>Please check in for <strong>{{event_name}}</strong>.</p>', $v(['applicant_name', 'event_name', 'event_date', 'event_location']), []),
      'event.volunteer.assigned' => $this->tpl('event-volunteer-assigned', 'Event volunteer assigned', 'events', 'event.volunteer.assigned', 'Volunteer assignment — {{event_name}}', '<p>Hello {{applicant_name}},</p><p>You have been assigned as {{role}} for <strong>{{event_name}}</strong>.</p>', $v(['applicant_name', 'event_name', 'role']), []),
      'event.schedule.change' => $this->tpl('event-schedule-change', 'Event schedule change', 'events', 'event.schedule.change', 'Schedule update — {{event_name}}', '<p>Hello {{applicant_name}},</p><p>The schedule for <strong>{{event_name}}</strong> has changed. {{reason}}</p>', $v(['applicant_name', 'event_name', 'reason', 'event_date', 'event_time']), []),
      'event.accommodation.confirmed' => $this->tpl(
        'event-accommodation-confirmed',
        'Accommodation confirmed',
        'events',
        'event.accommodation.confirmed',
        'Accommodation Confirmed — {{event_name}}',
        '<p>Hello {{applicant_name}},</p><p><strong>Accommodation Confirmed</strong></p><p>Type: {{accommodation_type}}<br>Location: {{location}}<br>Check-in: {{check_in}}<br>Check-out: {{check_out}}<br>Status: {{service_status}}</p><p>{{service_details_html}}</p>',
        $v(['applicant_name', 'event_name', 'accommodation_type', 'location', 'check_in', 'check_out', 'service_status', 'service_details_html', 'registration_number']),
        ['accommodation_type' => 'Shared 4-person room', 'location' => 'Campus Lodge', 'check_in' => '10 Nov', 'check_out' => '13 Nov', 'service_status' => 'Confirmed'],
      ),
      'event.transport.confirmed' => $this->tpl(
        'event-transport-confirmed',
        'Transportation confirmed',
        'events',
        'event.transport.confirmed',
        'Transportation Confirmed — {{event_name}}',
        '<p>Hello {{applicant_name}},</p><p><strong>Transportation Confirmed</strong></p><p>Pickup: {{pickup}}<br>Destination: {{destination}}<br>Vehicle: {{vehicle}}<br>Driver: {{driver}}<br>Contact: {{contact}}<br>Status: {{service_status}}</p><p>{{service_details_html}}</p>',
        $v(['applicant_name', 'event_name', 'pickup', 'destination', 'vehicle', 'driver', 'contact', 'service_status', 'service_details_html']),
        ['pickup' => 'Airport', 'destination' => 'Venue', 'vehicle' => 'Bus 12', 'driver' => 'James', 'contact' => '+10000000000'],
      ),
      'event.travel.confirmed' => $this->tpl(
        'event-travel-confirmed',
        'Travel assistance booked',
        'events',
        'event.travel.confirmed',
        'Travel Booked — {{event_name}}',
        '<p>Hello {{applicant_name}},</p><p><strong>Travel Assistance Booked</strong></p><p>Origin: {{pickup}}<br>Destination: {{destination}}<br>Travel dates: {{travel_dates}}<br>Status: {{service_status}}</p><p>{{service_details_html}}</p>',
        $v(['applicant_name', 'event_name', 'pickup', 'destination', 'travel_dates', 'service_status', 'service_details_html']),
        ['pickup' => 'Lagos', 'destination' => 'Accra', 'travel_dates' => '9–14 Nov'],
      ),
      'event.payment.verified' => $this->tpl('event-payment-verified', 'Event payment verified', 'events', 'event.payment.verified', 'Payment verified — {{event_name}}', '<p>Hello {{applicant_name}},</p><p>Your payment for <strong>{{event_name}}</strong> has been verified.</p><p>{{service_details_html}}</p>', $v(['applicant_name', 'event_name', 'service_details_html', 'registration_number']), []),
      'event.service.updated' => $this->tpl('event-service-updated', 'Event service updated', 'events', 'event.service.updated', 'Service update — {{event_name}}', '<p>Hello {{applicant_name}},</p><p>Your {{service_type}} request for <strong>{{event_name}}</strong> has been updated. Status: {{service_status}}</p><p>{{service_details_html}}</p>', $v(['applicant_name', 'event_name', 'service_type', 'service_status', 'service_details_html']), []),
      'event.service.cancelled' => $this->tpl('event-service-cancelled', 'Event service cancelled', 'events', 'event.service.cancelled', 'Service cancelled — {{event_name}}', '<p>Hello {{applicant_name}},</p><p>Your {{service_type}} request for <strong>{{event_name}}</strong> has been cancelled.</p>', $v(['applicant_name', 'event_name', 'service_type']), []),
      'auth.password.reset' => $this->tpl('auth-password-reset', 'Password reset', 'learning', 'auth.password.reset', 'Reset your Marketplace Ministers password', '<p>Hello {{applicant_name}},</p><p>Use this link to choose a new password:</p><p><a href="{{reset_url}}">Reset password</a></p><p>If you did not request this, you can ignore this email.</p>', $v(['applicant_name', 'email', 'reset_url', 'login_url']), []),
      'auth.email.verification' => $this->tpl('auth-email-verification', 'Verify your email', 'learning', 'auth.email.verification', 'Verify your email address', '<p>Hello {{applicant_name}},</p><p>Please confirm your email address to finish creating your account.</p><p><a href="{{verification_url}}">Verify email</a></p>', $v(['applicant_name', 'email', 'verification_url', 'login_url']), []),
      'form.business-review.submitted' => $this->tpl('form-business-review-submitted', 'Business review confirmation', 'contact', 'form.business-review.submitted', 'We received your business review application', '<p>Hello {{applicant_name}},</p><p>Thank you for submitting <strong>{{business_name}}</strong> for a Faith &amp; Works Business Review. Our team will follow up shortly.</p>', $v(['applicant_name', 'email', 'business_name']), ['applicant_name' => 'Founder', 'business_name' => 'Example Ltd']),
      'form.business-review.submitted.admin' => $this->tpl('form-business-review-admin', 'Business review admin alert', 'contact', 'form.business-review.submitted.admin', 'New business review — {{business_name}}', '<p>{{applicant_name}} ({{email}}) submitted a business review for <strong>{{business_name}}</strong>.</p>', $v(['applicant_name', 'email', 'business_name', 'phone']), []),
      'business-review.status.updated' => $this->tpl('business-review-status-updated', 'Business review status update', 'contact', 'business-review.status.updated', 'Business Review update', '<p>Hello {{applicant_name}},</p><p>{{message}}</p>', $v(['applicant_name', 'email', 'business_name', 'application_status', 'message']), []),
      'membership.application.rejected' => $this->tpl('membership-application-rejected', 'Membership application outcome', 'membership', 'membership.application.rejected', 'Membership application update', '<p>Hello {{applicant_name}},</p><p>Thank you for applying to Marketplace Ministers. After review we are not able to approve application <strong>{{application_number}}</strong> at this time.</p><p>{{reason}}</p>', $v(['applicant_name', 'email', 'application_number', 'reason']), []),
    ];
  }
